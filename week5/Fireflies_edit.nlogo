...
to check_sync ; observer
  if ((count turtles with [clock = reset-level]) = (count turtles))
  [
    set is_clock_synced? true
  ]
  if ((count turtles with [color = yellow]) = (count turtles)) and (is_clock_synced? = true)
  [
    set sync_time ticks
    set is_synced? true
  ]
...
@#$#@#$#@
<experiments>
  <experiment name="sync_time" repetitions="5" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="2000"/>
    <exitCondition>is_synced? = true</exitCondition>
    <metric>sync_time</metric>
    <steppedValueSet variable="cycle-length" first="5" step="1" last="20"/>
