globals
[
  ; q (set in slider) probability of butterflies moving according to elevation
  min_height
  max_height
...
to go
  ...
  [
    ...
    ; get final corridor width
    ; let final_corridor_width corridor_width
    ; type "Corridor width: " print final_corridor_width
    type "Number of Patches with Butterflies: " print turtles-area
    stop
...
to open_file
  ...
  set min_height (min [elevation] of patches)
  set max_height (max [elevation] of patches)
...
; initialise height
to set_height ; observer
  ask patches
  [
    ; set color
    set pcolor scale-color green elevation min_height max_height
...
; initialise turtles
to create_turtles ; observer
  crt 50
  [
    set size 2
    setxy (120 + random 10) (120 + random 10)
...
to move_turtles
  ...
  [
    ...
    [
      ; move to the highest neighbouring patch
      uphill elevation
    ]
    [
      ; randomly choose one neighbour to move to
...
; get corridor width
to-report corridor_width

  ; count number
  let visited count patches with [used? = true]

  ; calculate distance
  let dist_travelled (mean [distance start_patch] of turtles)

  ; calculate width
  report (visited / dist_travelled)

end

; get number of patches with turtles
to-report turtles-area

  ; count number
  report count patches with [ count turtles-here > 0 ]

end

; erase pen, toggled by button
to erase_pen ; observer
  ask turtles
  [
    pen-erase
  ]
...
This file describes the model of Pe弾r et al. (2005). The description is taken from Section 3.4 of Railsback and Grimm (2019). The file uses the markup language used by NetLogo's Info tab starting with NetLogo version 5.0.
...
The model has two kinds of entities: butterflies and square patches of land. The patches make up a square grid landscape of 150 ﾗ 150 patches, and each patch has one state variable: its elevation. Butterflies are characterized only by their location, described as the patch they are on. Therefore, butterfly locations are in discrete units, the x- and y- coordinates of the center of their patch. Patch size and the length of one time step in the simulation are not specified because the model is generic, but when real landscapes are used, a patch corresponds to 25 ﾗ 25 m<sup>2</sup>. Simulations last for 1000 time steps; the length of one time step is not specified but should be about the time it takes a butterfly to move 25・5 m (the distance from one cell to one of its neighbor cells).
...
To allow _observation_ of the two patterns used to define the model痴 usefulness, we use graphical display of topography and butterfly locations. Observing virtual corridors requires a specific 田orridor width・measure that characterizes the width of butterfly paths from their starting patches to hilltops.

## 5. Initialization
The topography of the landscape (the elevation of each patch) is initialized when the model starts. Two kinds of landscapes are used in different versions of the model: (1) a simple artificial topography, and (2) the topography of a real study site, imported from a file containing elevation values for each patch. The butterflies are initialized by creating five hundred of them and dispersing them throughout the landscape: each butterfly痴 initial location is set to a patch selected randomly from among all patches.

## 6. Input Data
The environment is assumed to be constant, so the model has no input data.

## 7. Submodels
The movement submodel defines exactly how butterflies decide whether to move uphill or randomly. First, to 杜ove uphill・is defined specifically as moving to the neighbor patch that has the highest elevation; if two patches have the same elevation, one is chosen randomly. 溺ove randomly・is defined as moving to one of the neighboring patches, with equal probability of choosing any patch. 哲eighbor patches・are the eight patches surrounding the butterfly痴 current patch. The decision of whether to move uphill or randomly is controlled by the parameter _q_, which ranges from 0.0 to 1.0 (_q_ is a global variable: all butterflies use the same value). On each time step, each butterfly draws a random number from a uniform distribution between 0.0 and 1.0. If this random number is less than _q_, the butterfly moves uphill; otherwise, the butterfly moves randomly.

## CREDITS AND REFERENCES
Pe弾r, G., Saltz, D. & Frank, K. 2005. Virtual corridors for conservation management. _Conservation Biology_, 19, 1997・003.

Pe弾r, G. 2003. Spatial and behavioral determinants of butterfly movement patterns in topographically complex landscapes. Ph.D. thesis, Ben-Gurion University of the Negev.
